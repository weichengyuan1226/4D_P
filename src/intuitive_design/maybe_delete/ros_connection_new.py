...
from intuitive_robotic_fabrication.fabrication import Point
...
class Sensordata():
    ...
    def set_buttons(self, sticky):
        controller_buttons = sticky
        self.name = controller_buttons['header']['frame_id']
        self.buttons = controller_buttons['buttons']
        self.axes = {'trigger' : controller_buttons['axes'][0], 'trackpadX': controller_buttons['axes'][1], 'trackpadY': controller_buttons['axes'][2]}
        return self.name, self.axes, self.buttons
    ...
    def pose_calc_tf(self, sticky_name):
...
#transforming frame-data from ROS
    ...
    def transform_pose_list(self, controller_frame):
        ...
        self.t_plane = rs.PlaneFromFrame(t_frame[0], t_frame[1],t_frame[2])
        self.origin= [t_frame[0].x, t_frame[0].y, t_frame[0].z]
        return self.t_plane, self.origin
